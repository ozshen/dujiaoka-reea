...
<!-- 发卡详情  -->
<div class="col-lg-7 text-white bill faka">
    ...
    <div class="row mb-3">
        <div class="iphone">
            ...
            <form class="form">
                ...
                <fieldset>
                    <!-- 支付方式 -->
                    <legend>{{ __('bing.orderinfo_payment_method') }}</legend>

                    <div class="form__radios">
                        <div class="form__radio">
                            <label for="visa"><svg class="icon">
                                    <use xlink:href="#icon-visa" />
                                </svg>{{ $order['pay']['pay_name'] ?? ''  }}</label>
                            <input checked id="visa" name="payment-method" type="hidden" />
                        </div>
                    </div>
                </fieldset>
                ...
                <div>
                    ...
                    <table>
                        <tbody>
                            <tr>
                                <!-- 邮箱 -->
                                <td>{{ __('bing.orderinfo_email') }}</td>
                                <td align="right">{{ $order['email'] }}</td>
                            </tr>

                            <!-- 下单时间 -->
                            <tr>
                                <td>{{ __('bing.orderinfo_order_time') }}</td>
                                <td align="right">{{ $order['created_at'] }}</td>
                            </tr>

                            <!-- 订单类型 -->
                            <tr>
                                <td>{{__('bing.orderinfo_order_class')}}</td>
                                @if($order['type'] == \App\Models\Order::AUTOMATIC_DELIVERY)
                                <td align="right">{{__('bing.orderinfo_automatic_delivery')}}</td>
                                @else
                                <td align="right">{{ __('bing.orderinfo_charge') }}</td>
                                @endif
                            </tr>
                            ...
                            <tr>
                                ...
                                <td align="right">{{__('bing.global_currency')}}{{ $order['goods_price'] }}</td>
                            </tr>

                            <!-- 数量 -->
                            <tr>
                                <td>{{__('bing.bill_number')}}</td>
                                <td align="right">{{ $order['buy_amount'] }}</td>
                            </tr>

                            <!-- 订单状态 -->
                            <tr>
                                <td>{{ __('bing.orderinfo_order_status') }}</td>
                                <td align="right" class="text-white">
                                    @switch($order['status'])
                                    @case(\App\Models\Order::STATUS_EXPIRED)
                                    {{-- 已过期 --}}
                                    {{ __('bing.orderinfo_status_expired') }}
                                    @break
                                    @case(\App\Models\Order::STATUS_WAIT_PAY)
                                    {{-- 待支付 --}}
                                    {{ __('bing.orderinfo_status_wait_pay') }}
                                    @break
                                    @case(\App\Models\Order::STATUS_PENDING)
                                    {{-- 待处理 --}}
                                    {{ __('bing.orderinfo_status_pending') }}
                                    @break
                                    @case(\App\Models\Order::STATUS_PROCESSING)
                                    {{-- 已处理 --}}
                                    {{ __('bing.orderinfo_status_processed') }}
                                    @break
                                    @case(\App\Models\Order::STATUS_COMPLETED)
                                    {{-- 已完成 --}}
                                    {{ __('bing.orderinfo_status_completed') }}
                                    @break
                                    @case(\App\Models\Order::STATUS_FAILURE)
                                    {{-- 已失败 --}}
                                    {{ __('bing.orderinfo_status_failed') }}
                                    @break
                                    @case(\App\Models\Order::STATUS_FAILURE)
                                    {{-- 状态异常 --}}
                                    {{ __('bing.orderinfo_status_abnormal') }}
                                    @break
                                    @endswitch
                                </td>
                            </tr>
                        ...
                        <tfoot>
                            <tr>
                                <td>总金额</td>
                                <td align="right">{{__('bing.global_currency')}}{{ $order['actual_price'] }}</td>
                            </tr>
                        </tfoot>
                    </table>
                </div>
            ...
            {{-- 卡密 --}}
            <div class="km">
                <p class="title">{{__('bing.orderinfo_carmi')}}</p>
                <textarea name="" id="" class="col-12" rows="5">
                {{$order['info']}}
                </textarea>
            </div>
            <button class="btn btu-clone btn-warning" data-clipboard-text="{{$order['info']}}">
                {{__('bing.orderinfo_copy_carmi')}}
            </button>
        </div>
    </div>
    @endforeach
    ...
    <svg xmlns="http://www.w3.org/2000/svg" style="display: none">
        ...
        <symbol id="icon-visa" viewBox="0 0 1024 1024">
            <path d="M520.2 520m-450 0a450 450 0 1 0 900 0 450 450 0 1 0-900 0Z" fill="#72E798" opacity=".6" p-id="4524"></path>
            <path d="M572.6 367.8l-7.7-23c-7.2-21.2-30.1-32.7-51.4-25.5l-144.7 48.6h203.8z m0 0" fill="#13D48D" p-id="4525"></path>
            <path d="M654.4 376.1l-7.8-23.2c-7.2-21.3-30.3-32.9-51.7-25.7L449.4 376h205z m0 0" fill="#13D48D" p-id="4526"></path>
            <path d="M700.7 722.8H339.6c-24.1 0-43.7-19.6-43.7-43.7V399.5c0-24.1 19.6-43.7 43.7-43.7h361.2c24.1 0 43.7 19.6 43.7 43.7v279.6c-0.1 24.2-19.6 43.7-43.8 43.7z m0 0" fill="#13D48D" p-id="4527"></path>
            <path d="M626 580.3c-22.4 0-40.5-18.1-40.5-40.5s18.1-40.5 40.5-40.5l118.6-0.9v81.1l-118.6 0.8z m0 0" fill="#FFFFFF" p-id="4528"></path>
            <path d="M662.1 540.3c0 7.3 3.9 14 10.2 17.6 6.3 3.6 14 3.6 20.3 0 6.3-3.6 10.2-10.3 10.2-17.6 0-7.3-3.9-14-10.2-17.6-6.3-3.6-14-3.6-20.3 0a20.25 20.25 0 0 0-10.2 17.6z m0 0" fill="#13D48D" p-id="4529"></path>
        </symbol>
    </svg>
...
<script>
    var clipboard = new ClipboardJS(".btu-clone")
